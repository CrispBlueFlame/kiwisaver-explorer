...
Tidy CSV copies of the whole dataset live in [`data/csv/`](data/csv/): funds, providers, the FMA quarterly history, and pre-2013 market context. Every column is documented in [`DATA_DICTIONARY.md`](DATA_DICTIONARY.md). They are a convenience mirror of the JSON the site uses, regenerated by `python3 scripts/build-data.py`. Full provenance for every source is in [`kiwisaver-dataset/DATA_SOURCES.md`](kiwisaver-dataset/DATA_SOURCES.md).
